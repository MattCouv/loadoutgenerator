...
const Loadout = ({generate, loadout}) => (
      <div id="loadout" className="loadout-container-wpr">
        ...
        <div className="row">
          ...
          <h1><span><i></i>kit</span><span className="kitname">{loadout.kit.sid.replace('WARSAW_ID_M_', '')}</span></h1>
            </div>
            <div className="kit-picture box-content gen" onClick={generate.genAll}>
          <div className="picture" style={{background: `url(/assets/bf4/kits/${loadout.kit.sid.replace('WARSAW_ID_M_', '').toLowerCase()}.png) 0% 0% / 100%`} }></div>
            </div>
            <div className="gen box-content" onClick={generate.genAll}><div className="gen-border"><h1><span>generate</span></h1></div></div>
          </div>
